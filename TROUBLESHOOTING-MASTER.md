# 🧯 全課程 14 專案防翻車與卡點終極救援總手冊（講師與學生必備）

> **這份手冊的目的**：把課堂上、自學時「所有可能卡住的點」一次盤點清楚。
> 不論遇到環境問題、埠號衝突、誤刪資料、還是沒有網路/API Key，**只要看這份手冊，30 秒內一定能解決！**
...
├────────────────────────────────────────────────────────────────────────────┤
│ 2. 遇到連不上網路、沒有付費 API Key：                                         │
│    👉 全 14 專案皆內建「100% 離線 Mock 模式」，零花費、零網路依然順暢放映！    │
├────────────────────────────────────────────────────────────────────────────┤
...
| **Project 9**<br>RAG 知識庫 | 沒有 OpenAI API Key 或網路斷線 | 檢索或評估時連線超時 | 執行 `./project-9/demo.sh 9`，系統使用本機內建語料進行 **100% 離線評估**，10 題全綠通過 |
| **Project 10**<br>MCP Server | 執行時提示找不到 `dist/index.js` | 模組載入失敗 | 執行 `npm run build`，TypeScript 會自動編譯輸出到 `dist/` |
| **Project 11**<br>Phaser 遊戲 | 角色掉出地圖或速度過快穿牆 | 物理碰撞失效 | 執行 `./project-11/demo.sh 1` 跑 `physics-check.mjs`，物理限速會自動鎖定在 400px/s 防穿牆 |
...
| **Project 13**<br>AutoCV YOLO | 沒有 GPU 或 CUDA 記憶體不足 | PyTorch 拋出 OOM | 設定檔 `wafer.yaml` 已預設 CPU 輕量模式（`imgsz: 640`, `batch: 8`），一般筆電均可秒跑 |
| **Project 14**<br>GraphRAG | 沒有 Neo4j 密碼或連不上 | 圖資料庫連線失敗 | 腳本內建 NetworkX 本機記憶體圖譜備援，無需 Docker 也能跑出知識圖譜多跳推理 |
...
1. 打開該專案的 **`walkthrough.md`**（例如 [Project 2 Walkthrough](./project-2/walkthrough.md)）。
