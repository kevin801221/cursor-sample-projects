...
> 程式碼在 `./shortenurl/`，遙控器是 `./demo.sh`（位於 `project-5-fastapi-backend/` 根目錄）。
...
| # | 做什麼 | 為什麼要先做 |
|---|---|---|
| 1 | `cd project-5-fastapi-backend/shortenurl && uv sync --extra dev` | 第一次同步 uv 虛擬環境與下載依賴。課前做完後，課堂上全離線秒開 |
