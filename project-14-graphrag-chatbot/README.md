...
| **技術棧** | Python 3.12 + uv、Gemini API（免費）、Chroma（向量資料庫）、Neo4j 5（知識圖譜；地端 Docker 首選，Aura Free 為替代）、FastAPI、React 力導向圖 |
| **預估時間** | 第一次完整跑約 60–90 分鐘（含環境）；之後 30 分鐘內 |
| **前置需求** | Docker Desktop、Node.js 18+、免費 Google AI Studio 金鑰（aistudio.google.com）。裝不了 Docker → 用 Neo4j Aura Free 雲端替代（walkthrough Step 0-3B 有完整替代路線） |
...
# 裝不了 Docker？改設 NEO4J_URI="neo4j+s://xxxx.databases.neo4j.io"（Aura Free，
# 完整步驟見 walkthrough Step 0-3B），腳本一行都不用改。
